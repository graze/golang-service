...
func TestOffset(t *testing.T) {
	...
	assert.Equal(t, 0, p1.GetOffset())

	p2, _ := New(3, 10, 10, &http.Request{})
	assert.Equal(t, 20, p2.GetOffset())

	p3, _ := New(3, 5, 10, &http.Request{})
	assert.Equal(t, 10, p3.GetOffset())
}
...
func TestPageUrl(t *testing.T) {
	...
	assert.Equal(t, "http://example.com:80/path/to/data/?foo=bar&limit=10&page=1&test=test", p.getURL(1).String())
}
