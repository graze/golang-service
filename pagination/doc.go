...
/*
Package pagination allows you to break up large sets of information (i.e., HTTP responses) into smaller,
more manageable ones.

Pagination adds complexity to both the service and its clients. The decision to paginate a resource should
...
The `pagination` attribute contains metadata about how the resource is paginated, including a list of URLs
for the client(s) to navigate between paged resources. The general idea is to limit the amount of work a
client has to do to implement a paginated API. e.g. avoid clients having to build URLs
...
The `page` URL parameter is an integer that represents the page indices of a given resource.
e.g. `https://api.com/resource?page=12`,
...
The `limit` URL parameter is an integer that specifies the number of items that should be returned in a page.
This should be reflected in the first, last, next, and previous URLs.
